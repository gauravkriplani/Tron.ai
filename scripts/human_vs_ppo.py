...
# Allow running as: `python scripts/human_vs_ppo.py` without `pip install -e .`
...
from tron_ai.envs.opponents import PPOOpponent
...
def main() -> None:
    ...
    ppo_opp = PPOOpponent(checkpoint_path=args.model, device=args.device)
    env = TronEnv(grid_size=checkpoint_grid_size, max_steps=args.max_steps, opponent_policy=ppo_opp, render_mode=None)

    viewer = PygameTronViewer(grid_size=checkpoint_grid_size, cell_size=args.cell, fps=args.fps)
